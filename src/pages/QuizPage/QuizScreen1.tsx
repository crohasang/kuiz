import { TopBackground, BottomBackground, ButtonContainer, Title, Button, Container, ExplanationContainer, NextButton } from './quizscreen1_styles';
...
function QuizScreen1() {
  ...
  const handleOptionClick = (option: string) => {
    if(isEnabled) { //버튼이 활성화된 경우에만 클릭 이벤트를 수행
  ...
  return (
    <Container>
      ...
      <TopBackground className={(selectedAnswer !== null) ? 'top-background-animate' : ''}>
        <Title>2/5</Title>
        <Title color="#ffffff">{getTitleText()}</Title>
      </TopBackground>
      {/* 하단 배경 및 콘텐츠 */}
      <BottomBackground>
        <ButtonContainer>
          <Button 
            style={{ backgroundImage: `url(${option1})`, backgroundSize: '100% 100%', pointerEvents: isEnabled ? 'auto' : 'none'}}
            onClick={() => handleOptionClick('A')}
            className={selectedAnswer === null ? '' : (correctAnswer === 'A' ? 'correct' : 'incorrect')} //정답이 'A' 옵션인 경우 'correct', 아닌 경우 'incorrect'를 className으로 넘긴다
          >
            <span className="overlay-text">A</span>
          </Button>
          <Button 
            style={{ backgroundImage: `url(${option2})`, backgroundSize: '100% 100%', pointerEvents: isEnabled ? 'auto' : 'none'}}
            onClick={() => handleOptionClick('B')}
            className={selectedAnswer === null ? '' : (correctAnswer === 'B' ? 'correct' : 'incorrect')} //정답이 'B' 옵션인 경우 'correct', 아닌 경우 'incorrect'를 className으로 넘긴다
          >
